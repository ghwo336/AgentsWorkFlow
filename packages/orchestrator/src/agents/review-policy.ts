...
export const QUALITY_LENS = [
  ...
  "- Then audit engineering quality, FAILing only when the violation is MATERIAL",
  "  (it will actively hurt the next steps or maintenance). Explicitly check for:",
  "  - SRP: god-functions/files/components doing many unrelated jobs the step",
  "    could reasonably separate — e.g. one unit holding data fetching + business",
  "    logic + rendering, or a module accumulating a second unrelated purpose",
  "  - DRY: the diff COPY-PASTES logic that already exists in the repo (verify the",
  "    original actually exists before failing — search, don't guess)",
  "  - Layering: UI reaching into DB directly when the project has a data layer,",
  "    circular imports, abstractions bypassed, business logic dumped into route",
  "    handlers/controllers when the project keeps a service layer",
  "  - Error handling: errors swallowed (empty catch, .catch(()=>{}), broad",
  "    except: pass) or failures silently converted to defaults that downstream",
  "    code will misread as success",
  "  - Type-safety erosion: any/@ts-ignore/unchecked casts used to silence the",
  "    compiler where a real type was available",
  "  - Convention drift: the diff invents a NEW pattern (state management, naming,",
  "    folder layout, styling approach) where the repo already has an established",
  "    one for the same job",
  "  - Misleading names, dead code, unused exports that will confuse the next step",
  "  - OVER-engineering is a violation too: speculative abstraction with a single",
  "    implementation, indirection layers or config options for requirements the",
  "    plan never asked for (YAGNI)",
...
export const SECURITY_LENS = [
  ...
  "  - XSS: user-controlled data reaching innerHTML/dangerouslySetInnerHTML/v-html",
  "    or otherwise rendered unescaped",
  "  - CSRF on state-changing routes; CORS wildcard combined with credentials;",
  "    session/token cookies missing httpOnly/secure/sameSite; tokens in localStorage",
  "  - Hardcoded or leaked secrets, credentials, tokens, private keys — including",
  "    server secrets shipped to clients (NEXT_PUBLIC_*, mobile app bundles)",
  "  - Missing authentication / authorization or broken access control — including",
  "    object-level checks (IDOR): a route that verifies login but never checks the",
  "    resource belongs to the requester",
  "  - Trusting the client: validation only on the client, JWT decoded without",
  "    signature verification, authorization decisions made client-side",
  "  - Mass assignment: raw request body spread into ORM create/update data",
  "  - Unvalidated/untrusted input reaching sensitive sinks (fs, network, shell, DB)",
  "  - Mobile: tokens/PII stored in plaintext (UserDefaults/AsyncStorage/",
  "    SharedPreferences instead of Keychain/Keystore/EncryptedSharedPreferences),",
  "    cleartext-traffic exceptions (ATS exceptions, usesCleartextTraffic)",
